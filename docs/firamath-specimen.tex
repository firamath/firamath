...
\clist_const:Nn \c_@@_unicode_reserved_clist
  {
    % Greek and Coptic
...
\clist_const:Nn \c_@@_unicode_control_clist
  {
...
\dim_const:Nn \c_@@_nuni_wd_dim        { 87 pt }
...
\cs_new_protected:Npn \@@_glyph:n #1
  {
    ...
    \clist_if_in:NVTF \c_@@_unicode_reserved_clist \l_tmpa_tl
      { \cellcolor { reserved@bg } }
      {
        \clist_if_in:NVTF \c_@@_unicode_control_clist \l_tmpa_tl
...
\cs_new_protected:Npn \@@_non_unicode:nn #1#2
  {
    \tl_set:Nn \l_@@_glyph_count_tl { \ref* { count: glyph @ nuni } }
    \section { #1 ~ (\tl_use:N \l_@@_glyph_count_tl) }
    \cs_set_eq:NN \@@_left_foot: \prg_do_nothing:
    \int_gzero:N \g_@@_nuni_glyph_int
    \@@_nuni_import:n {#2}
    \@@_nuni_parse:
    \@@_nuni_print:
    \newcounter     {     glyph @ nuni }
    \int_set:cn     { c @ glyph @ nuni } { \g_@@_nuni_glyph_int - 1 }
    \refstepcounter {     glyph @ nuni } \label { count: glyph @ nuni }
    \refstepcounter {     glyph        } \label { count: glyph        }
  }
\int_new:N \g_@@_nuni_glyph_int

% #1 = file name (non-unicode.txt)
\cs_new_protected:Npn \@@_nuni_import:n #1
  {
    \ior_open:Nn \g_@@_nuni_ior {#1}
    \ior_map_inline:Nn \g_@@_nuni_ior
      { \seq_put_right:Nn \l_@@_nuni_data_seq {##1} }
    \ior_close:N \g_@@_nuni_ior
  }
\ior_new:N \g_@@_nuni_ior
\seq_new:N \l_@@_nuni_data_seq

% `;` indicates a comment, for the glyph class.
% Each entry (glyph name) after the comment should be put into the class.
\cs_new_protected:Npn \@@_nuni_parse:
  {
    \bool_until_do:nn { \seq_if_empty_p:N \l_@@_nuni_data_seq }
      {
        \seq_pop_left:NN \l_@@_nuni_data_seq \l_tmpa_tl
        \tl_if_in:NnTF \l_tmpa_tl { ; }
          {
            \tl_remove_once:Nn \l_tmpa_tl { ; }
            \tl_trim_spaces:N \l_tmpa_tl
            \seq_put_right:NV \l_@@_nuni_class_seq \l_tmpa_tl
            \clist_if_empty:NF \l_tmpa_clist
              {
                \seq_put_right:NV \l_@@_nuni_name_seq \l_tmpa_clist
...
\seq_new:N \l_@@_nuni_class_seq
\seq_new:N \l_@@_nuni_name_seq

\cs_new_protected:Npn \@@_nuni_print:
  {
    \bool_until_do:nn { \seq_if_empty_p:N \l_@@_nuni_class_seq }
      {
        \seq_pop_left:NN \l_@@_nuni_class_seq \l_tmpa_tl
        \seq_pop_left:NN \l_@@_nuni_name_seq  \l_tmpa_clist
        \@@_nuni_print_class:VV \l_tmpa_tl \l_tmpa_clist
      }
  }

% #1 = class
% #2 = clist of glyph names
\cs_new_protected:Npn \@@_nuni_print_class:nn #1#2
  {
    ...
    \clist_map_inline:nn {#2}
      {
        \@@_nuni_cell:n {##1}
        \refstepcounter { glyph }
        \int_gincr:N \g_@@_nuni_glyph_int
      }
  }
\cs_generate_variant:Nn \@@_nuni_print_class:nn { VV }

\cs_new_protected:Npn \@@_nuni_cell:n #1
  {
    \tabular { | C { \c_@@_nuni_wd_dim } | }
      % Glyph
      \hline
...
\newfontface\BMPFallbackFont{unifont-12.0.01.ttf}[Path=../assets/GNU_Unifont/]
\newfontface\SMPFallbackFont{unifont_upper-12.0.01.ttf}[Path=../assets/GNU_Unifont/]
%%<RELEASE>
